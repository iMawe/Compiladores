...
def p_sapaAssignmentList1(p):
	...
	p[0] = sapaAssignmentList1(Id(p[1]),Assign(p[2]),Numeros(p[3]),"sapaAssignmentList1")
	#print "sapaAssignmentList 1"

def p_sapaAssignmentList2(p):
	'''sapaAssignmentList : sapaAssignmentList COMMA ID ASSIGN NUMEROS'''
	p[0] = sapaAssignmentList2(p[1],Id(p[3]),Assign(p[4]),Numeros(p[5]),"sapaAssignmentList2")
...
def p_unanDecl1(p):
	...
	p[0] = unanDecl1(p[1],Id(p[3]),p[5],"unanDecl1")
	#print "unanDecl 1"
...
def p_relation2(p):
	'''relation : NOIGUALQ'''
	p[0] = relation2(Noigualq(p[1]),"relation2")
	#print "relation 2"

def p_relation3(p):
	'''relation : MENORQ'''
	p[0] = relation3(Menorq(p[1]),"relation3")
	#print "relation 3"

def p_relation4(p):
	'''relation : MAYORQ'''
	p[0] = relation4(Mayorq(p[1]),"relation4")
	#print "relation 4"

def p_relation5(p):
	'''relation : MENOIGUQ'''
	p[0] = relation5(Menoiguq(p[1]),"relation5")
	#print "relation 5"

def p_relation6(p):
	'''relation : MAYOIGUQ'''
	p[0] = relation6(Mayoiguq(p[1]),"relation6")
	#print "relation 6"
...
def p_addingOperator1(p):
	...
	p[0] = addingOperator1(Opesuma(p[1]),"addingOperator")
	#print "addingOperator 1"

def p_addingOperator2(p):
	'''addingOperator : OPEMENOS'''
	p[0] = addingOperator2(Operesta(p[1]),"subtractionOperator")
	#print "addingOperator 1"
...
def p_multiplyingOperator1(p):
	...
	p[0] = multiplyingOperator1(Opemult(p[1]),"multiplyingOperator")
	#print "multiplyingOperator 1"

def p_multiplyingOperator2(p):
	'''multiplyingOperator : OPEDIV'''
	p[0] = multiplyingOperator2(Opediv(p[1]),"divisiongOperator")
...
def p_factor2(p):
	'''factor : NUMEROS'''
	p[0] = factor2(Numeros(p[1]),"factor2")
	#print "factor 1"
